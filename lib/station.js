...
Station.prototype.sendStats = function () {
  ...
  vendors.mongo(function(db) {
    ...
    async.parallel([
      function(callback){
        collection.find({ date: { $lt: now, $gt: dayBefore }, level: 'warned' }).toArray(function(err, triggers) {
          callback(undefined, triggers.length);
        });
      },
      function(callback){
        collection.find({ date: { $lt: now, $gt: dayBefore }, level: 'alarmed' }).toArray(function(err, triggers) {
          callback(undefined, triggers.length);
        });
      },
      function(callback){
        collection.find({ date: { $lt: now, $gt: dayBefore }, level: 'fired' }).toArray(function(err, triggers) {
          callback(undefined, triggers.length);
        });
      },
      function(callback){
        collection2.find({ date: { $lt: now, $gt: dayBefore }}).toArray(function(err, triggers) {
          callback(undefined, triggers.length);
        });
      }
    ],
